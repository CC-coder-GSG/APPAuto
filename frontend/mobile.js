...
async function mobileLogin(){
  ...
  try{const r=await fetch('/auth/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','X-Client-Type':'mobile'},body});if(!r.ok)throw new Error('用户名或密码错误');mobileToken=(await r.json()).access_token;localStorage.setItem('mobile_token',mobileToken);await mobileBoot()}catch(e){loginError.textContent=e.message}
}
