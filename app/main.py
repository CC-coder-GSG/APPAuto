...
def reports_summary(
    ...
    if all_users_mode:
        overview = {
            "executed_requirements": (
                db.query(func.count(func.distinct(TestExecution.id)))
            ...
            "created_cases": (
                db.query(func.count(TestCase.id))
                .filter(TestCase.created_at >= sdt, TestCase.created_at <= edt)
                .scalar()
                or 0
            ),
            "created_bugs": (
            ...
            "retested_reqs": (
                db.query(func.count(Requirement.id))
                .filter(Requirement.retested_at >= sdt, Requirement.retested_at <= edt)
                .scalar()
                or 0
            ),
            "closed_bugs": (
                db.query(func.count(BugTracking.id))
                .filter(BugTracking.closed.is_(True), BugTracking.updated_at >= sdt, BugTracking.updated_at <= edt)
                .scalar()
                or 0
            ),
        }
    ...
    while cur <= end_date:
        ...
        if all_users_mode:
            day_exec = (
                db.query(func.count(func.distinct(TestExecution.id)))
                .filter(TestExecution.executed_at >= day_s, TestExecution.executed_at <= day_e)
                .scalar()
                or 0
            )
            day_case = (
                db.query(func.count(TestCase.id))
                .filter(TestCase.created_at >= day_s, TestCase.created_at <= day_e)
                .scalar()
                or 0
            )
            day_bug = (
                db.query(func.count(BugTracking.id))
                .filter(BugTracking.created_at >= day_s, BugTracking.created_at <= day_e)
                .scalar()
                or 0
            )
            day_retested = (
                db.query(func.count(Requirement.id))
                .filter(Requirement.retested_at >= day_s, Requirement.retested_at <= day_e)
                .scalar()
                or 0
            )
            day_closed = (
                db.query(func.count(BugTracking.id))
                .filter(BugTracking.closed.is_(True), BugTracking.updated_at >= day_s, BugTracking.updated_at <= day_e)
                .scalar()
                or 0
            )
        else:
