...
def list_versions(
    ...
    _ = current_user
    rows = db.query(Version).order_by(Version.version_type.asc(), Version.version_no.asc()).all()
    return [
        {
            "id": v.id,
...
def create_requirement(
    ...
    existing = db.query(Requirement).filter(Requirement.internal_id == payload.internal_id).first()
    if existing:
        raise HTTPException(status_code=400, detail="internal_id exists")
    ...
    return {
        "id": requirement.id,
        "internal_id": requirement.internal_id,
